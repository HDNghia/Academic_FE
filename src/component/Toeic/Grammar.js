import Add from "./Grammar/Add";
import Update from "./Grammar/Update";
import { useEffect, useState } from "react";
import "../Responsive.css"
import axios from "axios";
import moment from "moment";
import Revise from "./Grammar/Revise";
function Grammar() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?date=${dateQuesToday}&&part=Grammar&&subject=english`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?subject=english&&part=Grammar&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?subject=english&&part=Grammar`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?question=${search}&&subject=english&&part=Grammar&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
