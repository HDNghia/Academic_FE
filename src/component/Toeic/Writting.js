import Add from "./Writting/Add";
import Update from "./Writting/Update";
import { useEffect, useState } from "react";
import "../Responsive.css"
import axios from "axios";
import moment from "moment";
import Revise from "./Writting/Revise";
function Writting() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?date=${dateQuesToday}&&part=Writting&&subject=english`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?subject=english&&part=Writting&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?subject=english&&part=Writting`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?question=${search}&&subject=english&&part=Writting&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
