...
function Vocabulary() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:3000/v1/question?date=${dateQuesToday}&&part=vocabulary&&subject=english`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:3000/v1/question?subject=english&&part=vocabulary&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:3000/v1/question?subject=english&&part=vocabulary`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`http://localhost:3000/v1/question?question=${search}&&subject=english&&part=vocabulary&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
