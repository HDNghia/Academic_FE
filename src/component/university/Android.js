...
function Android() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?date=${dateQuesToday}&&part=theory&&subject=Android`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?subject=Android&&part=theory&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?subject=Android&&part=theory`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`http://localhost:8080/api/v1/question?question=${search}&&subject=Android&&part=theory&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
