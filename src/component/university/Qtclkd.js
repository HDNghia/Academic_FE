...
function Qtclkd() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?date=${dateQuesToday}&&part=theory&&subject=Qtclkd`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?subject=Qtclkd&&part=theory&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?subject=Qtclkd&&part=theory`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`http://localhost:8080/api/v1/question?question=${search}&&subject=Qtclkd&&part=theory&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
