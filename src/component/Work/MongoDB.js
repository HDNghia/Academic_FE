import Add from "./MongoDB/Add";
import Update from "./MongoDB/Update";
import { useEffect, useState } from "react";
import "../Responsive.css"
import axios from "axios";
import moment from "moment";
import Revise from "./MongoDB/Revise";
function MongoDB() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?date=${dateQuesToday}&&part=theory&&subject=MongoDB`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?subject=MongoDB&&part=theory&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?subject=MongoDB&&part=theory`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`https://hdnghia-academic-be-9ybl.onrender.com/v1/question?question=${search}&&subject=MongoDB&&part=theory&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
                        ...
                        && QuestionToday.map((item, index) => {
                            return (
                                <>
                                    ...
                                    {index === count && QuestionToday[count].numberDate <= 21 && QuestionToday[count].subject === 'MongoDB' ? <>
