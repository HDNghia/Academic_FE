...
function Sql() {
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?date=${dateQuesToday}&&part=theory&&subject=Sql`)
            setQuestionToday(
                res.data.data
            )
        }
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?subject=Sql&&part=theory&&page=${page}&&limit=9`);
            setListQuestion(
                res.data.data
            )
    ...
    useEffect(() => {
        async function fetchApi() {
            let res = await axios.get(`http://localhost:8080/api/v1/question?subject=Sql&&part=theory`);
            setLengthQuestion(
                res.data.data
            )
        }
    ...
    const Search = async (event) => {
        ...
        let res = await axios.get(`http://localhost:8080/api/v1/question?question=${search}&&subject=Sql&&part=theory&&page=${page}&&limit=9`);
        setListQuestion(
            res.data.data
        )
    }
